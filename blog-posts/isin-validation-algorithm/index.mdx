...
```python
#this algoritm checks and sum validates the ISIN based on the structure and numbers, this can be cross checked with the proper isin_string

def isin_check_sum_validate(isin_string):
    if len(isin_string) != 12 or not all(check_string.isalpha() for check_string in isin_string[:2]) or not all(check_string.isalnum() for check_string in isin_string[2:]):
        return False
    string_full = "".join(str(int(check_string, 36)) for check_string in isin_string)
    return 0 == (sum(sum(divmod(2 * (ord(check_string) - 48), 10)) for check_string in string_full[-2::-2]) +
                 sum(ord(check_string) - 48 for check_string in string_full[::-2])) % 10
...
print ([isin_check_sum_validate(string_full) for string_full in ["ES0109067019", "GB0002374006", "HK0941009539", "US037833100G",
                         "CNE1000007Q1", "XA0109067019", "ES0109067019"]])
```
