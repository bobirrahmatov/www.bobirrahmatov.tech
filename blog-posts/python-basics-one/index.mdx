...
Python is one of the fastest growing programming languages. It is becoming so much popular and being used in everywhere, from healthcare to the self-driving cars.
...
In order to install Python, first download the latest version or specific release from the website (https://www.python.org/downloads/) based on your operating system. Please make sure to download the appropriate version of Python for your operating system.

![Python downloads window](/assets/images/blog-posts/python-download.png)

Once the package is download, simply follow the installation instructions shown on the installation window:
...
It is recommended having a shell alias to execute python3 each time you run python.
...
In order to configure the pip, create an `.ini` file in the directory `C:\Users\[username]\pip\pip.ini`
...
#once above is done, please try to check if pip is working
pip install pandas
```
